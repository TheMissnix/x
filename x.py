    ...
    try:
        ...
        if req1.text.__contains__('"id": "'):
            ...
            if response.__contains__('succeeded'):
                print(
                    GREEN+f''' [CHARGED]  =>   {lista}   Message =>  Charged card  - @predator_incoming''')
                with open("CHARGED.txt", 'a') as sav:
                    sav.write(
                        f''' [CHARGED]  =>   {lista}   Message =>  Charged card  - @predator_incoming''')
                    sav.write('\n')
            elif response.__contains__("intent_confirmation_challenge"):
                print(
                    RED + f''' [DEAD] ❌ =>   {lista}   Message => Hcaptcha triggered (change stripe link and use HQ porxy)- @predator_incoming''')
            elif response.__contains__("Your card's security code is incorrect"):
                print(
                    GREEN+f''' [LIVE CCN]  =>   {lista}   Message =>  Card security code is incorrect - @predator_incoming''')
                with open("CCN.txt", 'a') as sav:
                    sav.write(
                        f''' [LIVE CCN]  =>   {lista}   Message =>  Card security code is incorrect - @predator_incoming''')
                    sav.write('\n')
            elif response.__contains__("Your card has insufficient funds"):
                print(
                    GREEN+f''' [LIVE CC]  =>   {lista}   Message =>  Your card has insufficient funds - @predator_incoming''')
                with open("INSUFF.txt", 'a') as sav:
                    sav.write(
                        f''' [LIVE CC]  =>   {lista}   Message =>  Your card has insufficient funds - @predator_incoming''')
                    sav.write('\n')
            elif response.__contains__("Your card does not support this type of purchase"):
                print(
                    GREEN+f''' [LIVE CC]  =>   {lista}   Message =>  Your card does not support this type of purchase - @predator_incoming''')
                with open("TRANSCATIONxNxA.txt", 'a') as sav:
                    sav.write(
                        f''' [LIVE CC]  =>   {lista}   Message =>  Your card does not support this type of purchase  - @predator_incoming''')
                    sav.write('\n')
            else:
                mssg = response_dict2["error"]["message"]
                dec = response_dict2["error"]["decline_code"]
                print(
                    RED + f''' [DEAD] ❌ =>   {lista}   Message => {mssg}  decline_code => {dec}  - @predator_incoming''')
        else:
            message = response_dict["error"]["message"]
            print(
                RED + f''' [DEAD] ❌ =>   {lista}   Message => {message} - @predator_incoming''')
